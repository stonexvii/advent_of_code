...
class Number:
    ...
    def __str__(self):
        return f'{self.value} | {self.row} x {self.index}'
            ...
            if not (cur_x is None or cur_y is None):
                if (k != 0 or n != 0) and matrix[cur_x][cur_y].isdigit():
                    if not a_coords or (
                        ...
                        a_coords.append(Number(cur_x, cur_y))
    return a_coords
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix[i])):
            if matrix[i][j] not in '1234567890.':
                coordinates += adjacent_coords(matrix, (i, j))
    return coordinates
...
def sum_matrix(nums: list[Number], matrix: [list[str]]) -> int:
    result = []
    for num in nums:
        check_number(num, matrix)
        if num not in result:
            result.append(num)
    [print(num) for num in sorted(result, key=lambda x: x.value)]
    return sum([int(item.value) for item in result])
